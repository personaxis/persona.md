...
> **Archived target:** OpenClaw/SOUL.md export is archived for now. Active setup guides are Claude Code (`docs/setup/claude-code.md`) and Codex (`docs/setup/codex.md`). This document remains for historical reference.
...
```bash
npx @personaxis/persona.md compile --root --target soul-md
```

This generates `SOUL.md` from your `.personaxis/personaxis.md` in the format OpenClaw expects. Inject it as the agent's soul at startup — OpenClaw reads `SOUL.md` first and shapes all subsequent behavior from it.
...
- The user can open `.personaxis/personaxis.md` to review your interpretation and adjust any field that does not match their intent. After any change, re-run `personaxis compile --root --target soul-md` to regenerate SOUL.md.
...
This creates the persona in `.personaxis/personas/<slug>/` and compiles it to `SOUL.md` for that specific agent role.
...
- If `.personaxis/personaxis.md` (or `.personaxis/personas/<slug>/personaxis.md`) changes, re-run `personaxis compile --target soul-md` (with `--root` for the project baseline) to regenerate SOUL.md.
